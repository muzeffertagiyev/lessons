...
document.addEventListener('DOMContentLoaded',function(){
  form.onsubmit = function(event){
    event.preventDefault();//if we dont write that after submiting our context will dissapear because of submit's work
    const value = parseInt(amount.value);//making string integer
    const random = Math.floor(Math.random()*text.length)
    
    //empty
    //-1
    //>9
    if(isNaN(value) || value <= 0 || value > 9){//is (NaN-not a number)
        result.innerHTML = `<p class="result">${text[random]}</p>`
    }
    else{
      let tempText = text.slice(0,value); // it grabes values from 0 to submited value from text array
      tempText = tempText.map(function(paragraph){//if we do not use this method all paragrapsh will come before each other ,they will not start from new line, and it is like collapsing array into paragraphs
        return `<p class="result">${paragraph}</p>`;
      }).join("");//join is used for removing commas between paragraph
